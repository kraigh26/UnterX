module MarketplaceRouter
  module DataTypes
    ...
    # If URL, route_name is not needed
  ...
  #
  # Example, return hash with url without protocol:
  #
  # { url: "marketplace.sharetribe.com/listings", status :found }
  #
  # Example, return hash with named route:
  #
  # { route_name: :new_community, status: :moved_permanently }
  #
  ...
  def redirect_target(reason:, request:, community:, paths:, configs:)
    target =
      ...
      when :new_marketplace
        ...
        paths[:new_community].merge(status: :found)
      when :not_found
        # Community not found
      ...
      when :domain
        ...
        {url: "#{community[:domain]}#{request[:port_string]}#{request[:fullpath]}", status: :moved_permanently}
      when :no_domain
        # Community has a domain, but it's not in use.
        # -> Redirect to subdomain (ident)
        {url: "#{community[:ident]}.#{configs[:app_domain]}#{request[:port_string]}#{request[:fullpath]}", status: :moved_permanently}
      when :www_ident
